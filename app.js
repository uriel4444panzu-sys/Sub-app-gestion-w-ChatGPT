...
const FIREBASE_CONFIG_OVERRIDE_KEY = "subpilot-firebase-config";
const CALENDAR_REMINDER_DAYS = [7, 3, 1];
const FIREBASE_SDK_VERSION = "12.7.0";
const FIREBASE_CONFIG_VERSION = "24";
...
const connectionSetup = document.querySelector("#connectionSetup");
const firebaseSetupForm = document.querySelector("#firebaseSetupForm");
const setupApiKey = document.querySelector("#setupApiKey");
const setupAuthDomain = document.querySelector("#setupAuthDomain");
const setupProjectId = document.querySelector("#setupProjectId");
const setupAppId = document.querySelector("#setupAppId");
...
firebaseSetupForm.addEventListener("submit", handleFirebaseSetupSubmit);
...
function loadFirebaseServices() {
  return loadFirebaseConfigModule().then((configModule) => {
    const config = { ...(configModule.firebaseConfig || {}), ...loadStoredFirebaseConfig() };
...
function loadStoredFirebaseConfig() {
  const config = parseJson(localStorage.getItem(FIREBASE_CONFIG_OVERRIDE_KEY), {});
  return isFirebaseConfigComplete(config) ? config : {};
}

function handleFirebaseSetupSubmit(event) {
  event.preventDefault();
  const config = {
  ...
  if (!isFirebaseConfigComplete(config)) {
    authStatus.textContent = "Complétez les 4 champs de connexion avant de réessayer.";
    return;
  }

  localStorage.setItem(FIREBASE_CONFIG_OVERRIDE_KEY, JSON.stringify(config));
  authStatus.textContent = "Configuration enregistrée, connexion en cours…";
  firebaseState = { ...firebaseState, ready: false, configured: false, error: null };
  renderAccountStatus();
  initializeFirebaseAuth();
}
...
function renderAccountStatus(message) {
  ...
  connectionSetup.hidden = firebaseState.configured || Boolean(user);
