...
Si la page affiche que la connexion sécurisée est indisponible, cela signifie que la configuration Web Firebase publiée est vide ou obsolète. L’écran d’authentification propose alors un bloc **Activer la connexion** permettant de coller temporairement les quatre valeurs Web Firebase (`apiKey`, `authDomain`, `projectId`, `appId`) sur l’appareil de test, sans stocker de mot de passe. Pour la production, remplissez toujours `firebase-config.js` puis republiez l’application.
